...
from load import load_dataset, UKBiobankDataset
...
try:
    for epoch in tqdm(range(epochs), desc="Epochs"):
        ...
        batch_accuracy_score = []
        with torch.no_grad():
            for batch_x, batch_y in test_loader:
                output = model(batch_x)
                _, predicted = torch.max(output, 1)
                batch_accuracy_score.append(accuracy_score(batch_y.numpy(), predicted.numpy()))
        print("PyTorch MLP Accuracy:", np.mean(batch_accuracy_score))
